...
class ClassroomForm(forms.ModelForm):
    """Form to create/edit classroom."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Teacher in charge: Teacher or Admin accounts
        self.fields['teacher'].queryset = User.objects.filter(
            Q(role='teacher') | Q(role='admin') | Q(is_superuser=True)
        ).order_by('last_name', 'first_name')
        self.fields['teacher'].label = "Teacher in charge"
    ...
    class Meta:
        ...
        widgets = {
            'name':        forms.TextInput(attrs={
                'class': 'form-control', 'placeholder': 'Class name...', 'maxlength': '255',
            }),
            'tuition':     forms.NumberInput(attrs={
                'class': 'form-control', 'placeholder': 'e.g., 1200000', 'min': '0',
            }),
...
class ScheduleForm(forms.ModelForm):
    """Form for setting class schedule."""
    class Meta:
        model  = Schedule
        ...
        widgets = {
            ...
            'room_name':   forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'e.g., Room 201 (leave blank if online)',
...
class EnrollmentForm(forms.ModelForm):
    """Form to enroll student in classroom."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
